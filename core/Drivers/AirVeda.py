...
#             self.end_time=end
#             self.start_time = start.replace(tzinfo=pytz.UTC)
#             self.end_time = end.replace(tzinfo=pytz.UTC)
...
class AirVeda(AirnetDriverAbs):
    ...
    def process(self, deviceObj, dag_param):
        for dev in deviceObj:
            self.fetch_cal(deviceObj=dev)
            self.device_id = dev.device_id
            paramList = dag_param if dag_param else dev.parameters.split(",")
    ...
    def creating_df(self, deviceObj, request):
        try:
            ...
            df['time'] = pd.to_datetime(df['time'], format="%Y-%m-%d %H:%M:%S").dt.tz_localize('UTC').dt.tz_convert('Asia/Kolkata')
            if not self.time_added:
                self._df_list.append(df[['time']])
                self.time_added = True
    ...
    def postprocess(self, deviceObj):
        try:
            self._df_all = pd.concat(self._df_all_list) if self._df_all_list else pd.DataFrame()
        except Exception as e:
            logger.error(f"Error in postprocess: {e}")
    ...
    def fetch_cal(self, deviceObj):
        try:
            ...
            response = self.restPOST(cal_req, deviceObj) if self._fetch_method == 'POST' else self.restGET(cal_req, deviceObj)
            self._cal_http_response = response.json()
            ...
            # Filter the DataFrame between the start and end times
            cal_df = cal_df[(cal_df['time'] >= self.start_time) & (cal_df['time'] <= self.end_time)]
            
            cal_df.reset_index(inplace=True, drop=True)
            self._cal_df_list.append(cal_df)
            
            
        except Exception as e:
            logger.error(f"Error in fetch_cal: {e}")
